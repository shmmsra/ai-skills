...
**Date**: 2026-05-26
**Status**: Accepted
**Decider**: Shivam Mishra + AI agent (session 2026-05-26)

---

## Context

ai-skills will be developed by humans collaborating with multiple AI coding agents (Claude, Codex, Cursor, Gemini, future tools) across many sessions over an extended timeline. Without a structured workflow:

- Each new agent session starts from zero context — no shared rules, no shared status, no shared history.
- Agents propose plausible-looking changes that violate unstated constraints (skill structure, install-script conventions, template format).
- "Done" is ambiguous — changes ship without updated docs or recorded reasoning.
...
1. **Agent-config layer**: `CLAUDE.md`, `AGENTS.md`, `GEMINI.md` — all thin adapters pointing to one canonical `docs/agents/` triad. No rule duplication.
...
4. **Pre-commit gate**: `make check` runs shell-script syntax validation; pre-commit hook installed via `make setup-hooks` enforces locally.
...
6. **In-repo ticket tracking** (flat Markdown file in the repo — agents read it directly without external API access) — agents read tickets without external API access.
...
- **GitHub Issues as the canonical ticket store**: requires external API access for agents, adds context-switching overhead. In-repo wins for a solo/small-team repo.
